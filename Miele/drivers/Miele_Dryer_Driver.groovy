...
@Field static final Map DRYER_POWER_STATES = [
    "running": 2500,        // High power for heating and tumbling
    "in use": 2500,
    "heating": 3000,        // Peak power during heating phase
    "drying": 2500,
    "cooling": 500,         // Fan only during cool-down
    "pause": 50,
...
def installed() {
    parent.driverLogInfo(device.displayName, "Installed Miele Dryer Driver")
    initialize()
}

def updated() {
    parent.driverLogInfo(device.displayName, "Updated Miele Dryer Driver")
    initialize()
}
...
def on() {
    parent.driverLogDebug(device.displayName, "Turning on dryer", logEnable)
    ...
    if (parent.driverExecuteCommand(device.deviceNetworkId, "powerOn", commandBody)) {
        ...
        if (txtEnable) parent.driverLogInfo(device.displayName, "Dryer powered on")
    }
}

def off() {
    parent.driverLogDebug(device.displayName, "Turning off dryer", logEnable)
    ...
    if (parent.driverExecuteCommand(device.deviceNetworkId, "powerOff", commandBody)) {
        ...
        if (txtEnable) parent.driverLogInfo(device.displayName, "Dryer powered off")
    }
}

// Program control commands with robust error handling
def startProgram() {
    parent.driverLogDebug(device.displayName, "Starting dryer program", logEnable)
...
def pauseProgram() {
    parent.driverLogDebug(device.displayName, "Pausing dryer program", logEnable)
...
def stopProgram() {
    parent.driverLogDebug(device.displayName, "Stopping dryer program", logEnable)
...
def refresh() {
    parent.driverLogDebug(device.displayName, "Refreshing dryer state", logEnable)
...
def receiveMieleState(Map stateData) {
    ...
    try {
        ...
        stateData.each { String key, value ->
            processDryerStateUpdate(key, value, previousOperationState)
        }
...
private void processDryerStateUpdate(String key, value, String previousOperationState) {
...
private void handleOperationStateChange(String newState, String previousState) {
    if (previousState != newState) {
        ...
        parent.driverUpdatePowerEstimation(device, newState, DRYER_POWER_STATES, powerEstimation)
...
private String determineSwitchState(String operationState) {
    ...
    if (lowerState.contains("running") || lowerState.contains("in use") || lowerState.contains("heating") || 
        lowerState.contains("drying") || lowerState.contains("cooling") || lowerState.contains("pause") ||
        lowerState.contains("programmed")) {
        return "on"
